...
#else   // DEBUG
    fprintf(stdout, "s21_grep: %s: File processing error\n", dp->error_file);
#endif  // DEBUG
  }
}
  ...
  for (int i = 0; dp->t_files[i]; i++) {
    ...
    while ((read_bytes = getline(&line, &line_len, fp)) != -1 && !dp->errcode) {
      ...
      if (!dp->errcode && dp->templs[dp->templs_num - 1] == NULL)
        ...
        s21_strcpy(dp->templs[dp->templs_num - 1], line);
    }
    free(line);
...
void error_print(data_t* dp) {
  if (dp->errcode == 1) {
    fprintf(stdout, "s21_grep: System memory access error\n");
  } else if (dp->errcode == 2) {
    fprintf(stdout, "s21_grep: Too few arguments\n\n\n");
  } else if (dp->errcode == 3) {
    fprintf(stdout, "s21_grep: Illegal option -- %c\n", dp->error_ch);
    fprintf(stdout, "usage: s21_grep [-%s] [pattern] [file ...]\n", dp->shopts);
  } else if (dp->errcode == 4) {
    fprintf(stdout, "s21_grep: %s: No such file or directory\n",
            dp->error_file);
  } else if (dp->errcode == 5) {
    fprintf(stdout, "s21_grep: Option requires an argument -- %c\n",
            dp->error_ch);
    fprintf(stdout, "usage: s21_grep [-%s] [pattern] [file ...]\n", dp->shopts);
  } else if (dp->errcode == 6) {
    fprintf(stdout, "s21_grep: Target file is required\n");
    fprintf(stdout, "usage: s21_grep [-%s] [pattern] [file ...]\n", dp->shopts);
  } else if (dp->errcode == 7) {
    fprintf(stdout, "s21_grep: %s: File processing error\n", dp->error_file);
  }
