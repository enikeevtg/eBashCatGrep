...
/*==============================================================================
                                  s21_cat
==============================================================================*/
void s21_cat(int argc, char** argv, data_t* dp) {
...
void opt_def(int argc, char** argv, data_t* dp) {
  ...
  for (int i = 1; i < argc && !(dp->errcode) && !stop_argv; i++) {
    s21_size_t argvi_len = s21_strlen(argv[i]);  // Длина строки argv[i]
...
void shopt_ident(char* argvi, data_t* dp) {
  ...
  while (argvi[++i] && !dp->errcode) {
    char* opt_pos = s21_strchr(dp->shopts, argvi[i]);
    if (opt_pos == S21_NULL) {
      dp->errcode = 1;
      dp->error_ch = argvi[i];
    } else
...
void lopt_ident(char* argvi, data_t* dp) {
  ...
  for (int i = 0; i < dp->lopts_num; i++) {
    if (s21_strcmp(argvi, dp->lopts[i]) == 0)
      dp->opt_mask[i] = 1;
    else
      match--;
  }
...
void error_print(data_t* dp) {
  if (dp->errcode == 1) {  // ОБРАБОТКА ОШИБКИ 1 illegal option
    ...
    fprintf(stderr, "usage: s21_cat [-%s] [file ...]", dp->shopts);
  } else if (dp->errcode == 3)
    fprintf(stderr, "s21_cat: system memory access errcode");
}
