...
// cc -Wextra -Wall -Werror -pedantic -Ofast -o s21_cat Objects/main.c.o
...
//#define BUFFER_SIZE 64  // Для ускорения чтения файла, если s21_cat без флагов
...
//         printf("usage: s21_cat [-%s] [file ...]", shopts);
//         // fprintf(stderr, "s_21cat: illegal option -- %c\n", error_ch);
//         // fprintf(stderr, "usage: s21_cat [-%s] [file ...]", shopts);
//     }
//     if (errcode == 2)
...
//     char* opt_pos = s21_strchr(pdata->shopts, *argvi);
//     if (opt_pos == NULL) {
//       pdata->errcode = 1;
