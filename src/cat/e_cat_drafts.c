...
// cc -Wextra -Wall -Werror -pedantic -Ofast -o e_cat Objects/main.c.o -LDependencies/jz/ -ljz
...
//  Начинал писать распечатку в терминал с использованием нестандартных библиотек.
//  В тележном чате этого проекта было обсуждение по поводу необходимости использования таких библиотек,
//  если в стандартнй все нужные функции уже имеются.
//  Начало функции print2stdout через стандартную библиотеку (не стал использовать, так как низкоуровневый ввод-вывод
//  позволяет обращаться к файлам через дескриптор, и stdin в этом случае файл с дескрипотором 0)
    // FILE* fp;
...
// /*=========================================================================================================
//         Распечатка, реализованная через низкоуровневые функции ввода/вывода библиотеки <unistd.h>
// =========================================================================================================*/
...
// =========================================================================================================*/
// void print2stdout(int argc, char** argv, int FILE_index, bool* flags_mask, int* error, char** error_filename) {
//     int fd = STDIN_FILENO;  //  Файловый дескриптор fd = 0, т.е. по умолчанию
//                             //  при отсутствии имени файла будем читать stdin
//     for (int i = FILE_index; i <= argc && *error == 0; i++) {
//         if (FILE_index != argc)  //  если пользователь ввёл хотя бы одно имя файла
//             fd = open(argv[i], O_RDONLY);
//         if (fd < 0) {  //  если функция open() не смогла открыть файл и вернуло значение -1
//             *error = 2;
...
//         //  [NULL...Backspace] || [Vertical Tab...Unit Separator] -> ['@', 'A',...,'H'] || ['K'...'_']
...
//         prev_symb = symb;
//         if (symb == '\n' && print_access)  //  В случае наличия флага -s print_access здесь...
//             line_counter++;                //  ...учитывает сжатие при расчёте номера строки
//         print_access = TRUE;
//     }
// }
...
// =========================================================================================================*/
// void error_printing(int error, char error_ch, char* error_filename, char* shopts) {
//     if (error == 1) {
//         char error_message_illflag[36] = "";
//         char error_message_illflag_usage[32] = "usage: e_cat [-%s] [file ...]";
//         sprintf(error_message_illflag, "s_21cat: illegal option -- %c\n", error_ch);
//         sprintf(error_message_illflag_usage, "usage: e_cat [-%s] [file ...]", shopts);
//         write(STDERR_FILENO, error_message_illflag, 36);
//         write(STDERR_FILENO, error_message_illflag_usage, 32);
//     }
//     if (error == 2) {
//         char error_message_nofile[101] = "";
//         sprintf(error_message_nofile, "\ns_21cat: %s:no such file or directory", error_filename);
//         write(STDERR_FILENO, error_message_nofile, 36);
//     }
// }
...
//         //fprintf(stderr, "s_21cat: %s:no such file or directory", error_filename);  //  При выводе
//         // нескольких файлов одновременно, где второй файл несуществует, за сообщением об этой ошибке
